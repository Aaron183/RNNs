...
print('Evaluate RNN...')
model = Sequential()
model.add(SimpleRNN(hidden_units,activation='tanh',input_shape=x_train.shape[1:]))
model.add(Dense(num_classes))
model.add(Dropout(0.2))
model.add(Activation('softmax'))

model.compile(loss='binary_crossentropy',  optimizer=adam(LR),  metrics=['accuracy'])             
model.fit(x_train, y_train, batch_size=128, epochs=70,verbose=0, validation_data=(x_test, y_test))
